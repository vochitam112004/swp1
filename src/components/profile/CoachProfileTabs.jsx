...
import CoachAccountTab from "./tabs/CoachAccountTab";
import CoachSmokingHabitsTab from "./tabs/CoachSmokingHabitsTab";
...
function TabPanel(props) {
  ...
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`coach-profile-tabpanel-${index}`}
      aria-labelledby={`coach-profile-tab-${index}`}
      {...other}
    >
      {value === index && (
        <Box sx={{ p: 3 }}>
...
function a11yProps(index) {
  return {
    id: `coach-profile-tab-${index}`,
    'aria-controls': `coach-profile-tabpanel-${index}`,
  };
}
      ...
      <Paper className="profile-paper" elevation={6}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tabs 
            ...
            aria-label="coach profile tabs"
            variant="scrollable"
            scrollButtons="auto"
          >
            <Tab label="Thông tin tài khoản" {...a11yProps(0)} />
            <Tab label="Quản lý TriggerFactor" {...a11yProps(1)} />
          </Tabs>
        </Box>
